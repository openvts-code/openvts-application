...
class _LiveMapState extends ConsumerState<_LiveMap>
  ...
  Future<_ReplayRequestResult> _handleReplayRequested(
    ...
    try {
      final replay =
          await ref.read(liveMapVehicleServiceProvider).getVehicleReplayByImei(
                imei: imei,
                from: request.from,
                to: request.to,
                maxPoints: 5000,
...
class _VehicleLogsTabState extends ConsumerState<_VehicleLogsTab> {
  ...
  Future<void> _loadInitialLogs() async {
    ...
    try {
      final page =
          await ref.read(liveMapVehicleServiceProvider).getVehicleLogsByImei(
                imei,
                limit: _pageSize,
              );
  ...
  Future<void> _ensureSocketConnection() async {
    ...
    try {
      final connection =
          await ref.read(socketServiceProvider).connect('/telemetry');
      if (!mounted ||
  ...
  void _handleTelemetryUpdate(dynamic data) {
    final log =
        ref.read(liveMapVehicleServiceProvider).parseTelemetryLogPayload(data);
    if (log == null) {
      return;
    }
  ...
  void _handleTelemetrySnapshot(dynamic data) {
    final logs = ref
        .read(liveMapVehicleServiceProvider)
...
class _VehicleEventsTabState extends ConsumerState<_VehicleEventsTab> {
  ...
  Future<void> _loadInitialEvents() async {
    ...
    try {
      final page =
          await ref.read(liveMapVehicleServiceProvider).getVehicleEventsByImei(
                imei,
                limit: _pageSize,
              );
      if (!mounted || generation != _requestGeneration || _imei != imei) {
  ...
  Future<void> _ensureSocketConnection() async {
    ...
    try {
      final connection =
          await ref.read(socketServiceProvider).connect('/notifications');
      if (!mounted ||
          generation != _socketGeneration ||
          !widget.isActive ||
          _imei != imei) {
...
class _VehicleCommandsTabState extends ConsumerState<_VehicleCommandsTab> {
  ...
  Future<void> _loadCatalog() async {
    ...
    try {
      final service = ref.read(liveMapVehicleServiceProvider);
  ...
  Future<void> _pollCommandStatus(String cmdId, int generation) async {
    ...
    try {
      final status =
          await ref.read(liveMapVehicleServiceProvider).getCommandStatus(cmdId);
      if (!mounted || generation != _pollGeneration) {
        return;
      }
  ...
  void _showDetails(SuperadminVehicleCommandEntry entry) {
    ...
    final detailFuture = cmdId.isEmpty
        ? null
        : ref.read(liveMapVehicleServiceProvider).getCommandDetail(cmdId);
    showDialog<void>(
      context: context,
